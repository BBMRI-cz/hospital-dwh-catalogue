...
class TicketRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('requester_email', 'requester_name', 'subject', 'description')}),
        ('Status', {'fields': ('status', 'alvao_ticket_id', 'alvao_response')}),
        (
            'Timestamps',
            {'fields': ('created_at', 'updated_at', 'submitted_at'), 'classes': ('collapse',)},
        ),
    )
