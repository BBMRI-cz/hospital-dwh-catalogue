...
class CartView(CartMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        ...
        context['form'] = TicketSubmitForm(initial={
...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if not item_type or not item_id:
            return JsonResponse({
                'success': False,
                'error': _('Missing item type or ID')
            }, status=400)
        ...
        try:
            item_name, item_description, parent_dataset = self._get_item_details(
                item_type, item_id
            )
...
class SubmitCartView(CartMixin, FormView):
    ...
    def form_valid(self, form: TicketSubmitForm) -> HttpResponse:
        ...
        try:
            ...
            logger.info(
                f"Submitted ticket {response.ticket_id} for {cart.requester_email}"
            )
...
class MyTicketsView(ListView):
    ...
    def get_queryset(self) -> QuerySet[TicketRequest]:
        ...
        local_tickets = TicketRequest.objects.filter(
            ...
            status__in=[
                TicketRequest.Status.SUBMITTED,
                TicketRequest.Status.CONFIRMED
            ]
...
class CartCountView(CartMixin, View):
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        ...
        return JsonResponse({
            'count': self.get_cart_count(request)
        })
