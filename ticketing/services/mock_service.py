...
class MockAlvaoService:
    ...
    def _get_stored_ticket(self, ticket_id: str) -> Optional[dict]:
    ...
    def get_tickets_by_requester(self, requester_email: str) -> list[TicketInfo]:
        ...
        for ticket_id, data in MockAlvaoService._memory_storage.items():
        ...
        if self.use_database:
            ...
            db_tickets = TicketRequest.objects.filter(
                requester_email__iexact=requester_email,
                alvao_ticket_id__startswith='MOCK-'
            )
