"""Tests for the warehouse data-source models and services."""
...
from .services import WarehouseMetadataService
...
class WarehouseMetadataServiceTest(SimpleTestCase):
    """Focused tests for warehouse service-layer helper logic."""

    @patch.object(WarehouseMetadataService, 'get_column_counts_for_distributions')
    def test_build_column_counter_counts_once_per_dataset(self, mock_get_column_counts):
        mock_get_column_counts.return_value = [
            ('Age', 'dist_a'),
            ('Age', 'dist_b'),
            ('Age', 'dist_c'),
            ('Sex', 'dist_a'),
        ...
        counter = WarehouseMetadataService().build_column_counter(
        ...
        self.assertEqual(counter['Age'], 2)
        self.assertEqual(counter['Sex'], 1)
        mock_get_column_counts.assert_called_once_with(['dist_a', 'dist_b', 'dist_c'])
