...
class DatasetFilterForm(forms.Form):
    ...
    subject_tags = forms.MultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple(), label='Klíčová slova'
    )

    data_source = forms.MultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple(), label='Zdroj dat'
    )

    rights_holders = forms.MultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple(), label='Držitel práv'
    )
