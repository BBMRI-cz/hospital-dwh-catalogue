...
class FairGenomesScheduler:
    ...
    def _sync_loop(self):
        ...
        logger.info(f'Fair Genomes scheduler started (interval: {self.interval_hours} hours)')
    ...
    def start(self):
        ...
        self._thread = Thread(target=self._sync_loop, name='FairGenomesScheduler', daemon=True)
        self._thread.start()
        self._running = True

        logger.info(
            f'Fair Genomes scheduler thread started (next sync in {self.interval_hours} hours)'
        )
...
_scheduler: FairGenomesScheduler | None = None
